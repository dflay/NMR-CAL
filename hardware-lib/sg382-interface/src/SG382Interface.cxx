...
namespace SG382Interface { 
   ...
   int write_cmd(int type,int handle,const char *buffer){
      int rc = CommDriver::write_cmd(type,handle,buffer); 
      return rc;     
   }
   //______________________________________________________________________________
   int ask(int type,int handle,const char *query,char *response){
      int rc = CommDriver::query(type,handle,query,response);
      return rc; 
   }
   //______________________________________________________________________________
   int clear_error(int type,int handle){
      ...
      char cmd[SIZE];
      sprintf(cmd,"*CLS\n");
      int rc = CommDriver::write_cmd(type,handle,cmd); 
      return rc; 
   }
   //______________________________________________________________________________
   ...
   int set_bnc_output_state(int type,int handle,int flag) {
	 ...
	 sprintf(cmd,"ENBL%d\n",flag);  
         rc = CommDriver::write_cmd(type,handle,cmd); 
      }else{
   ...
   int get_bnc_output_state(int type,int handle,int &state){
      ...
      sprintf(cmd,"ENBL?\n");
      int rc = CommDriver::query(type,handle,cmd,response); 
      state = atoi(response); 
      return rc;  
   }
   //______________________________________________________________________________
